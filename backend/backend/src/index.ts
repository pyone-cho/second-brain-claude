...
// Run migrations eagerly in local/dev, lazily in serverless
let migrationsDone: Promise<void> | null = null;

function ensureMigrations(): Promise<void> {
  if (!migrationsDone) {
    migrationsDone = runMigrations().then(() => {
...
if (!isVercel) {
  // Local development: start server immediately
...
// For Vercel: ensure migrations run before first request
