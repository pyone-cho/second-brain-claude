...
export async function getItems(params: {
  ...
  const result = await db.execute({ sql, args: values as any[] });
...
export async function createItem(input: CreateItemInput): Promise<FullItem> {
  const id = uuidv4();
  ...
  // Prepare tag-related data
  const tagNames: string[] = (input.tags || []).map((t: string) => t.trim()).filter((t: string) => t.length > 0);

  // Build batch statements for transaction
  const statements = [
    {
      sql: `INSERT INTO items (id, type, status, pinned, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)`,
      args: [id, input.type, status, pinned, now, now] as any[],
    },
    {
      sql: `INSERT INTO ${detailTable} (${detailColumns.join(', ')}) VALUES (${detailPlaceholders.join(', ')})`,
      args: detailValues as any[],
    },
  ];
  ...
  // Execute item + detail + tag inserts in a transaction
  await db.batch(statements, 'write');

  // Now insert item_tags (need tag IDs first)
  for (const tagName of tagNames) {
...
export async function updateItem(id: string, input: UpdateItemInput): Promise<FullItem> {
  ...
  const updateTags = tagNames !== undefined && Array.isArray(tagNames);

  // Build batch for main updates
  const statements = [
    {
      sql: `UPDATE items SET ${itemUpdates.join(', ')} WHERE id = ?`,
      args: [...itemValues, id] as any[],
    },
  ];
  ...
  if (updateTags && tagNames.length > 0) {
    for (const tagName of tagNames) {
      ...
      const tag = tagResult.rows[0] as unknown as { id: number };
      if (tag) {
        await db.execute({
          sql: 'INSERT OR IGNORE INTO item_tags (item_id, tag_id) VALUES (?, ?)',
          args: [id, tag.id],
...
export async function updateItemStatus(id: string, newStatus: ItemStatus): Promise<FullItem> {
  const existing = await getItemById(id);
  if (!existing) {
    throw new Error('NOT_FOUND');
  }
