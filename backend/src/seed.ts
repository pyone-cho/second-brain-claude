...
async function insertItem(params: {
  ...
  const values: InArgs = [id, ...(Object.values(params.detailFields) as InValue[])];

  // Build batch for item + detail + tags
  const batch: Array<{ sql: string; args: InArgs }> = [];

  batch.push({
    sql: `INSERT INTO items (id, type, status, pinned, created_at, updated_at)
          VALUES (?, ?, ?, ?, ?, ?)`,
    args: [id, params.type, params.status, pinned, now, now],
  });

  batch.push({
    sql: `INSERT INTO ${params.detailTable} (${columns.join(', ')})
          VALUES (${placeholders})`,
    args: values,
  });
...
async function main() {
  ...
  await client.batch(
    [
      ...
      'DELETE FROM tags',
      'DELETE FROM tasks_ordinary',
      'DELETE FROM tasks_it_infra',
      'DELETE FROM readings',
      'DELETE FROM purchases',
      'DELETE FROM trips',
      'DELETE FROM items',
      'DELETE FROM categories',
      "DELETE FROM sqlite_sequence WHERE name IN ('categories', 'tags')",
    ],
    'write',
  );
  ...
  // Done
  const itemsCount = await client.execute('SELECT COUNT(*) as c FROM items');
  const categoriesCount = await client.execute('SELECT COUNT(*) as c FROM categories');
  const tagsCount = await client.execute('SELECT COUNT(*) as c FROM tags');

  console.log(`\n✅ Seed complete!`);
  console.log(`   📊 ${itemsCount.rows[0].c} items`);
  console.log(`   📁 ${categoriesCount.rows[0].c} categories`);
  console.log(`   🏷️  ${tagsCount.rows[0].c} tags`);
